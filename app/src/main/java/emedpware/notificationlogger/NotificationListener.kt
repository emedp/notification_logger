...
import emedpware.notificationlogger.database.Database
import emedpware.notificationlogger.database.MyNotification
...
class NotificationListener : NotificationListenerService() {
    ...
    override fun onNotificationRemoved(sbn: StatusBarNotification?, rankingMap: RankingMap?, reason: Int) {
        ...
        if (sbn != null) {
            ...
            // NOTIFY USER CODE BLOCK
            val alertDeletedNotification = preferences.getBoolean(keyAlertDeletedNotifications, true)

            if (alertDeletedNotification) {
                // category should be MSG or null
                if (notification.category == null || notification.category == Notification.CATEGORY_MESSAGE) {
                    // reason of the notification removed should be APP_CANCEL or APP_CANCEL_ALL
                    // TODO: the reason check don't work as expected (other user delete message so app delete notification)
